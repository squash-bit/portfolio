AOS.init();
/* Project Cards */

const projectcards = document.querySelector(".projectcards");

// Array of object for projects
const projects = [
  {
    title: "Agent Wallie",
    cardImage: "assets/images/project-page/agent-wallie.jpg",
    description: "Presenting an innovative Telegram bot that skillfully scrapes a News Site, like Hacker News, gathering captivating articles, news updates, and research papers that cater to the user's preferences. This advanced bot takes it a step further by expertly summarizing the content of each paper, creating a comprehensive report showcasing the article's title, concise summary, and a direct link. The user is guaranteed effortless access to the information they desire, as the bot automatically sends a copy of the report straight to their WhatsApp number. Embrace this intelligent solution for staying informed effortlessly!",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    Githublink: "https://github.com/squash-bit/Agent-Wallie",
  },
  {
    title: "Circum Mundum",
    cardImage: "assets/images/project-page/circum.jpg",
    description: "Introducing this sleek web application designed exclusively for ROSA KNUST, aimed at significantly reducing costs for our Circum Mundum Dinner. The app boasts an ingenious feature - the ability to generate a unique QR code for each registered user, streamlining the entire ticketing process. Additionally, it empowers the receptionist with a built-in QR reader, ensuring seamless verification of every ticket. All of this impressive functionality is made possible by the cutting-edge technology of Fastapi. Embrace this efficient solution, simplifying ticket management for our event!",
    tagimg: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Previewlink: "https://93mtzf.deta.dev/",
    Githublink: "https://github.com/Gaius-1/QRcode-Ticketing-System",
  },
  {
    title: "Intelligent Waste Segregator",
    cardImage: "assets/images/project-page/robot-arm.jpg",
    description: "Introducing a meticulously crafted Robotic Arm, meticulously designed to seamlessly sort any waste image into its appropriate bin, harnessing the remarkable potential of AI and Image Recognition. This groundbreaking creation served as the centerpiece of my final year project, a testament to the culmination of my dedication and expertise. Witness the seamless fusion of cutting-edge technology and environmental consciousness, as this state-of-the-art Robotic Arm contributes to a cleaner and more sustainable future.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    Githublink: "https://github.com/Gaius-1/Garbage-Bot",
  },
  {
    title: "Addo Grand Construction Website",
    cardImage: "assets/images/project-page/construction.png",
    description: "A company website for a Construction Firm, Addo Grand Construction.",
    tagimg: "https://miro.medium.com/max/2800/0*U2DmhXYumRyXH6X1.png",
    Previewlink: "https://addo-grand-construction.onrender.com/",
    Githublink: "",
  },
  {
    title: "Start Up Website, (In progress)",
    cardImage: "assets/images/project-page/Acecode-Website.png",
    description: "A company website for a startup company named AceCodeX.",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "https://gaius-1.github.io/AceCodeX",
    Githublink: "https://github.com/Gaius-1/AceCodeX",
  },
];

// function for rendering project cards data
const showCards = () => {
  let output = "";
  projects.forEach(({ title, cardImage, Previewlink, Githublink }) => {
    output += `       
        <div class="column skill-card card" style="margin: 15px"data-aos="zoom-in-up" data-aos-easing="linear" data-aos-delay="300" data-aos-duration="600" >
          <div class="wrapper" style="background: url(${cardImage}) center / cover no-repeat;">
            <div class="header">
            </div>
            <div class="data">
              <div class="content">
              <div class="title-div">
                <h1 class="title"><a href="#">${title}</a></h1>
                </div>
                <ul class="menu-content"><br>
                  <li><a href="${Previewlink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-monitor"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg></a></li>
                  <li><a href="${Githublink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-github"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg></a></li>
                </ul>
              </div>
            </div>
          </div>
        </div>`;
  });
  projectcards.innerHTML = output;
};
document.addEventListener("DOMContentLoaded", showCards);

function myFunction() {
  // Declare variables
  var input, button, i, skillcard, card, title;
  input = document.getElementById("myInput").value;
  input = input.toUpperCase();
  skillcard = document.getElementsByClassName("skill-card");
  card = document.getElementsByClassName("card");
  title = document.getElementsByClassName("title");

  // Loop through all list items, and hide those who don't match the search query
  for (i = 0; i < button.length; i++) {
    if (
      button[i].innerHTML.toUpperCase().includes(input) ||
      title[i].innerHTML.toUpperCase().includes(input)
    ) {
      skillcard[i].style.display = "";
      card[i].style.display = "";
    } else {
      skillcard[i].style.display = "none";
      card[i].style.display = "none";
    }
  }
}
